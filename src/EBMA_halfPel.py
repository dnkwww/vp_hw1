...
class ebma_halfPel:
    ...
    * MAD 與 PSNR 以 Y(亮度) 通道計算，padding 採 reflect
    ...
    def match(self):
        ...
        # 這支程式內的影像大小是 (H, W)
        mvx_interp = cv2.resize(mvx, (W, H), interpolation=cv2.INTER_NEAREST)
        mvy_interp = cv2.resize(mvy, (W, H), interpolation=cv2.INTER_NEAREST)
        ...
        cv2.imwrite(os.path.join(save_dir, 'ebma_halfPel_flow.jpg'),
