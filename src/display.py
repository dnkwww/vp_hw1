...
def displayFrame(bgr_in, flag, frameName, save_dir=''):
		base_dir = './results/'
		
		save_outputs = os.path.join(base_dir, f'test{flag}/')
		os.makedirs(save_outputs, exist_ok=True)

		# --- 影像處理與存檔 ---
		bgr_in = bgr_in.clip(0,255).astype(np.uint8)
		if len(save_dir) > 0:
			full_path = os.path.join(save_outputs, save_dir)
			cv2.imwrite(full_path, bgr_in)
